...
| `CONFIGURE_BUTTON_LABEL` | Свой текст кнопки «Настроить» (например «Подключиться»). Пусто = стандартный текст | `` |
| `CUSTOM_INSTRUCTION` | Кастомная инструкция в блоке подключения. Поддерживает `\n` для переноса строк | `` |
| `SHOW_TARIFF_DESCRIPTION` | Показывать описание тарифов в каталоге | `true` |
