...
import { IconQrcode, IconCopy, IconCheck, IconDownload, IconRefresh, IconTrash, IconPlus, IconPlayerStop, IconExchange, IconCreditCard, IconWallet, IconDeviceMobileCog, IconHelp } from '@tabler/icons-react';
...
import RemnawaveFaq from '../components/RemnawaveFaq';
...
export default function Services() {
  ...
  const filteredServices = services.filter((service) => {
    ...
    return (!query || service.service.name.toLowerCase().includes(query) || String(service.user_service_id).includes(query))
      && (statusFilter === 'all' || !statusFilter || service.status === statusFilter);
  });
  ...
  return (
    <Stack gap="lg" className="services-workspace">
      <Group justify="space-between">
        <Title order={2}>{t('services.title')}</Title>
        <Group>
          <Button leftSection={<IconHelp size={16} />} variant="default" onClick={() => setHelpOpen(true)}>
            Помощь
          </Button>
          <Button
      ...
      <Paper withBorder p="sm" radius="md" className="service-filters">
        <Group grow align="end">
          <TextInput label="Поиск услуги" placeholder="Название или ID" value={search} onChange={(event) => setSearch(event.currentTarget.value)} />
          <Select label="Статус" value={statusFilter} onChange={setStatusFilter} data={[{ value: 'all', label: 'Все статусы' }, ...Object.keys(statusColors).map((status) => ({ value: status, label: t(`status.${status}`, status) }))]} />
        </Group>
      </Paper>
        ...
        <Paper withBorder p="xl" radius="md">
          <Center>
            <Stack align="center" gap="md">
              <Text c="dimmed">{services.length ? 'По выбранным фильтрам услуги не найдены' : t('services.noServices')}</Text>
      ...
      <Modal
        ...
        title="Пошаговая помощь по Remnawave VPN"
        size="lg"
        className="remnawave-help-modal"
      >
        <RemnawaveFaq />
      </Modal>
