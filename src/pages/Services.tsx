...
import { IconQrcode, IconCopy, IconCheck, IconDownload, IconRefresh, IconTrash, IconPlus, IconPlayerStop, IconExchange, IconCreditCard, IconWallet, IconDeviceMobileCog, IconCalendar, IconServer, IconArrowRight } from '@tabler/icons-react';
...
function ServiceDetail({ service, onDelete, onChangeTariff }: ServiceDetailProps) {
  ...
  return (
    <Stack gap="md" className="service-detail">
      <Paper className="service-detail-hero" p="md">
        <Group justify="space-between" align="flex-start" wrap="nowrap">
          <Group gap="sm" wrap="nowrap">
            <Box className="service-detail-icon"><IconServer size={22} /></Box>
            <div style={{ minWidth: 0 }}>
              <Text fw={700} size="lg" truncate>{service.service.name}</Text>
              <Text size="xs" c="dimmed">ID #{service.user_service_id}</Text>
            </div>
          </Group>
          <Badge color={statusColor} variant="light" size="lg">{statusLabel}</Badge>
        </Group>
      ...
      <Tabs value={activeTab} onChange={setActiveTab}>
        ...
        <Tabs.Panel value="info" pt="md">
          <Stack gap="sm">
            <Paper className="service-info-panel" p="md">
              <Group justify="space-between" className="service-info-row"><Text size="sm" c="dimmed">{t('services.status')}</Text><Badge color={statusColor} variant="light">{statusLabel}</Badge></Group>
              <Group justify="space-between" className="service-info-row"><Text size="sm" c="dimmed">{t('services.cost')}</Text><Text size="sm" fw={600}>{service.service.cost} {t('common.currency')}</Text></Group>
              <Group justify="space-between" className="service-info-row"><Text size="sm" c="dimmed">ID</Text><Text size="sm" ff="monospace">#{service.user_service_id}</Text></Group>
            </Paper>
            {service.next && (
            ...
            {service.children && service.children.length > 0 && (
              <Paper className="service-children-panel" p="md">
                <Text size="xs" c="dimmed" mb="sm">{t('services.includedServices')}</Text>
                {service.children.map((child) => {
                  const childStatusColor = statusColors[child.status] || 'gray';
                  const childStatusLabel = t(`status.${child.status}`, child.status);
                  return (
                    <Group key={child.user_service_id} justify="space-between" className="service-child-row">
