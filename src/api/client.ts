...
export const userApi = {
  ...
  getPayments: (params?: { limit?: number; offset?: number; sort_field?: string; sort_direction?: string; start?: string; stop?: string; filter?: Record<string, unknown> }) => {
  ...
  getAutopayments: () => api.get('/user/autopayment'),
  getReferrals: () => api.get('/user/referrals'),
  getWithdrawals: (params?: { limit?: number; offset?: number; sort_field?: string; sort_direction?: string; start?: string; stop?: string; filter?: Record<string, unknown> }) => {
    const { filter, ...rest } = params || {};
