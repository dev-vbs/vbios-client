...
import { Modal, Stack, Text, Center, Loader, Paper, Pagination, LoadingOverlay, ScrollArea, Group, TextInput, Button } from '@mantine/core';
      ...
      {initialLoading ? (
        ...
        <Stack gap="md">
          <Group align="end" grow>
            <TextInput type="date" label="С" value={start} onChange={(event) => setStart(event.currentTarget.value)} />
            <TextInput type="date" label="По" value={stop} onChange={(event) => setStop(event.currentTarget.value)} />
            <Button variant="light" onClick={() => { setPage(1); fetchWithdrawals(1, false, sortField, sortDirection); }}>Применить</Button>
          </Group>
